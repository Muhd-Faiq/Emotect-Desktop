...
import re
import msvcrt
from auth import postEmotion

import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

from deepfaceT.deepface import DeepFace
from deepfaceT.deepface.extendedmodels import Age
from deepfaceT.deepface.commons import functions, realtime, distance as dst
from deepfaceT.deepface.detectors import FaceDetector
...
def analysis(db_path, model_name = 'VGG-Face', detector_backend = 'opencv', distance_metric = 'cosine', enable_face_analysis = True, source = 0, time_threshold = 5, frame_threshold = 5):
	...
	while(True):
				...
				if freezed_frame == 0:
					...
					for detected_face in detected_faces_final:
						...
						if enable_face_analysis == True:
							...
							print(str(int(apparent_age))," years old ", gender)
						...
						#tic is 5 seconds, so minus 2 is equal to 3
						tic = time.time()-2 #in this way, freezed image can show 5 seconds
		...
		if cv2.waitKey(1) & 0xFF == ord('\x1b'): #press esc to quit
			endtime=datetime.now().time().strftime("%H:%M:%S")
			print(endtime)
			postEmotion(emotion=emotionList,starttime=starttime,endtime=endtime,date=curdate)
			break
		
		cv2.setWindowTitle('img', 'Press ESC to exit')
